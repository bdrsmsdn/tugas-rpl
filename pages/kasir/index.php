...
$data_produk = mysqli_query($conn,"select p.id_pembayaran, p.nama_pelanggan, m.nama_minuman, p.total_pembayaran, p.tgl_pembayaran, p.status, ps.jumlah_pesanan from pembayaran as p join minuman as m join pesanan as ps WHERE ps.id_minuman = m.id_minuman and ps.id_pesanan = p.id_pesanan and p.status = 2 limit $halaman_awal, $batas");
				$nomor = $halaman_awal+1;
       ?> 
            <div class="row">
                <table class="table table-dark text-white text-center table-striped table-responsive-sm">
                <tr>
                    <th>No</th><th>ID Pembayaran</th><th>Nama Pelanggan</th><th>Nama Minuman</th><th>Jumlah</th><th>Total Bayar</th><th>Aksi</th>
                </tr>
                <?php
                $no = 1;
